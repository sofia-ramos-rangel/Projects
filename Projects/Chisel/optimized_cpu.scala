...
class PipelinedCPU(implicit val conf: CPUConfig) extends Module {
  ...
  // lab 5: flag to indicate the instruction coming into the CPU is multiplication
  //val mulFlag = Wire(UInt(1.W))

  // lab 5: flag to indicate that the booth8mul is done multiplying
  val mulDone = Wire(UInt(1.W))
  ...
  when ((mulDone === 0.U) || ~hazard.io.ifid_bubble) {
  ...
  // setting start signal for booth (mulFlag) if mul operation
  when (aluControl.io.operation === 11.U) {
	  booth.io.start := 1.U
  }
  .otherwise {
	  booth.io.start := 0.U
  ...
  mulDone := booth.io.done
  val done2 = booth.io.done2
  ...
  // lab 5: need to flush IF/ID stage when the instruction is multiplication to stall
  ...
  // lab 5: need to bubble when instruction is multiplication 
  ...
  when (mulDone === 0.U) {
    // Set the id_ex control to 0 to indicate a bubble
    id_ex.excontrol := id_ex.excontrol
    id_ex.mcontrol  := id_ex.mcontrol
    id_ex.wbcontrol := id_ex.wbcontrol
  }
  ...
  // lab 5: need to redirect inputy to go to booth8cycle instead of ALU
  // Input y mux (line 66 of single-cycle/cpu.scala)
  // alu.io.inputy := Mux(id_ex.excontrol.immediate, id_ex.imm, alu_inputy)

  val alu_inputy_2 = Wire(UInt(32.W))
  alu_inputy_2 := Mux(id_ex.excontrol.immediate, id_ex.imm, alu_inputy)
 ...
 alu.io.operation := aluControl.io.operation
  ...
  // when (mulFlag === 1.U && mulDone === 0.U) {
	  // alu.io.operation := 0.U
	  // aluControl.io.add := 0.U
	  // aluControl.io.immediate := 0.U
	  // aluControl.io.funct7    := 1.U
  ...
  // setting start and done booth io
  //booth.io.start := ~mulDone


  // set booth input y to alu input y
  alu.io.inputy 	:= alu_inputy_2
  booth.io.inputy 	:= alu.io.inputy
  
  // set booth.io.inputx to the alu input x
  alu.io.inputx 	:= alu_inputx
  booth.io.inputx 	:= alu.io.inputx
  ...
  // setting alurresult to either booth output or alu
  when (mulDone === 1.U && booth.io.done2 === 1.U) {
	  ex_mem.aluresult := booth.io.result
  }
  .otherwise {
	  ex_mem.aluresult := alu.io.result
  }
